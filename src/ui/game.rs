...
impl TryFrom<DisplayGameAction> for &'static str {
    ...
    fn try_from(value: DisplayGameAction) -> Result<Self, Self::Error> {
        ...
        match value.1 {
            TurnStatus::Wait => Err(()),
            TurnStatus::Ready(phase) => Ok(match (value.0, phase) {
                (
                    Cmd::SelectPrev,
                    Phase::DropAbility | Phase::SelectAbility | Phase::AttachMonster,
                ) => "SelectPrev",
                (
                    Cmd::SelectNext,
                    Phase::DropAbility | Phase::SelectAbility | Phase::AttachMonster,
...
impl<'a> Drawable for Hud<'a> {
    fn draw(&mut self, f: &mut Frame<Backend>, area: Rect) {
        ...
        f.render_widget(
            Paragraph::new(self.username).alignment(Alignment::Center),
            layout[0],
        );
...
impl<'a> Drawable for Monster<'a> {
    fn draw(&mut self, f: &mut Frame<Backend>, area: Rect) {
        f.render_widget(
            Paragraph::new(include_file_by_rank_and_suit! {from "monsters"
                match self.0.rank => {
                    Six
                    Seven
                    Eight
...
struct Monsters<'a>(
    &'a StatefulList<Option<Card>, [Option<Card>; 2]>,
    TurnStatus,
    Option<usize>, /*attack monster*/
);
...
impl<'a> Drawable for Monsters<'a> {
    fn draw(&mut self, f: &mut Frame<Backend>, area: Rect) {
               ...
               //     })
                {
                    ...
                    Monster(
                        ...
                        Style::default().fg(
                            if self.0.selected.is_some_and(|s| s == i)
        ...
        if self.1.is_ready_and(|p| p == GamePhaseKind::Defend) && self.2.is_some() {
            ...
            Monster(
                self.0.items[self.2.unwrap()].unwrap(),
                Some(
                    Block::default()
                        .borders(Borders::ALL)
                        .title("You Attacked by ...!"),
                ),
                Style::default(),
            )
            .draw(f, area);
...
impl Drawable for Ability {
    fn draw(&mut self, f: &mut Frame<Backend>, area: Rect) {
        f.render_widget(
            Paragraph::new(include_file_by_rank_and_suit!(from "abilities"
                match self.0.rank => {
                        Six
                        Seven
                        Eight
        ...
        let rect = rect_for_card_sign(
            area,
            SignPosition::new(VerticalPosition::Bottom, HorizontalPosition::Left),
        );
...
struct Abilities<'a>(
    Suit,
    &'a StatefulList<Option<Rank>, [Option<Rank>; 3]>,
    TurnStatus,
);
    ...
    fn draw(&mut self, f: &mut Frame<Backend>, area: Rect) {
        ...
        for (i, ability) in self.1.items.iter().enumerate() {
            ability.map(|ability| {
                ...
                let pad_v = layout[i]
                    .height
                    .saturating_sub(ABILITY_HEIGHT)
                    .saturating_div(2);
                let pad_h = layout[i]
                    .width
                    .saturating_sub(ABILITY_WIDTH)
                    .saturating_div(2);
                Ability(
                    Card::new(ability, self.0),
                    Style::default().fg(if self.1.selected.is_some_and(|s| s == i) {
                        Color::Cyan
                    } else if !matches!(
                        self.2,
                        TurnStatus::Ready(GamePhaseKind::SelectAbility)
...
impl Drawable for Rank {
    fn draw(&mut self, f: &mut Frame<Backend>, area: Rect) {
        [
            SignPosition::new(VerticalPosition::Top, HorizontalPosition::Left),
            SignPosition::new(VerticalPosition::Bottom, HorizontalPosition::Right),
        ]
        .iter()
...
impl Drawable for Suit {
    fn draw(&mut self, f: &mut Frame<Backend>, area: Rect) {
        [
            SignPosition::new(VerticalPosition::Top, HorizontalPosition::Right),
            SignPosition::new(VerticalPosition::Bottom, HorizontalPosition::Left),
        ]
        .iter()
